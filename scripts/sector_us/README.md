...
| 회차 | 실행 | 결과 | 소요 |
|---|---|---|---|
| 1 | 2026-09-12 07:19 UTC · 수동 실행 (`34680424005`) | 성공. `sector-us-latest.{md,json}` + 스냅샷 + `검증결과.{md,json}` 생성 | **전체 3분 10초** — 주간 수집 54초(14섹터 + 구성종목 102종목), §9 검증 1분 54초 |

1회차에서 드러난 것과 한 일:

| 드러난 것 | 한 일 |
|---|---|
| **Invesco CSV가 러너에서 실패**했다 (제안서 R3이 확인을 요구한 항목). 출처 ②`api.nasdaq.com`(A−)으로 폴백해 구성종목 102종목을 받았고 100종목의 가격이 붙었다 | 폴백까지의 실패 사유를 `brief.json.notes`와 `breadth.constituents_attempt_errors`에 남기게 했다. 1회차에는 폴백이 조용히 일어나 기록이 없었다 — 2회차 산출물에 사유가 적힌다 |
| 재현성 검증이 「불일치」만 남기고 **무엇이 달랐는지 안 적었다** | 티커·주·두 값·최대 상대차와 출처(대체 티커·가격 필드·행수·마지막 바) 변화를 함께 기록하게 했다 |
...
| 데이터 안정성 | 스냅샷 대비 과거 `rs_ratio` 변화 0.0%. 같은 기준일 두 번 수집은 **불일치**(원인 미확정) | 2회차부터 차이의 내용이 기록된다. 확정되기 전까지 이 줄은 「불일치」로 남는다 |
...
| 항목 | 상태 |
|---|---|
| 계산 정의 경계값 19건 | 통과 (`self_test.py`, 러너에서도 통과) |
